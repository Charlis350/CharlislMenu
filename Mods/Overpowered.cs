/*
 * Signal Menu  Mods/Overpowered.cs
...
namespace SignalMenu.Mods
{
    public static class Overpowered
    {
        ...
        private static float universalCrashDelay;
        private const int FLOOD_PACKETS = 2000;

        public static void MasterClientKickGun()
        {
            if (!PhotonNetwork.IsMasterClient)
            {
                NotificationManager.SendNotification("<color=grey>[</color><color=red>ERROR</color><color=grey>]</color> You must be master client to use this.");
                return;
            }
            ...
            if (GetGunInput(false))
            {
                ...
                if (gunLocked && lockTarget != null)
                {
                    Player targetPlayer = lockTarget.GetPhotonPlayer();
                    if (targetPlayer != null && !targetPlayer.IsLocal)
                    {
                        ...
                        NotificationManager.SendNotification($"<color=grey>[</color><color=green>KICKED</color><color=grey>]</color> {targetPlayer.NickName}");
                        gunLocked = false;
                    }
                }
        ...
        public static void MasterClientKickAll()
        {
            if (!PhotonNetwork.IsMasterClient)
            {
                NotificationManager.SendNotification("<color=grey>[</color><color=red>ERROR</color><color=grey>]</color> You must be master client to use this.");
            ...
            NotificationManager.SendNotification("<color=grey>[</color><color=green>SUCCESS</color><color=grey>]</color> Kicked all players from room.");
        }

        public static void FloodLagGun()
        {
            if (GetGunInput(false))
            {
                var GunData = RenderGun();
                ...
                if (gunLocked && lockTarget != null && Time.time > universalCrashDelay)
                {
                    universalCrashDelay = Time.time + 0.5f;
                    FloodCrashPlayer(GetPlayerFromVRRig(lockTarget));
                }
        ...
        public static void FloodLagAll()
        {
            if (rightTrigger > 0.5f && Time.time > universalCrashDelay)
            {
                universalCrashDelay = Time.time + 1f;
                foreach (NetPlayer player in NetworkSystem.Instance.PlayerListOthers)
                    FloodCrashPlayer(player);
            }
        }

        public static void FloodCrashPlayer(NetPlayer target)
        {
            if (target == null) return;

            int timestamp = PhotonNetwork.ServerTimestamp;
            int fakeViewId = PhotonNetwork.AllocateViewID(0);
            ...
            foreach (VRRig rig1 in VRRigCache.ActiveRigs)
            {
                ...
                foreach (VRRig rig2 in VRRigCache.ActiveRigs)
                {
                    ...
                    if (Vector3.Distance(rig2.leftHandTransform.position, rig1.headMesh.transform.position) < 0.25f || Vector3.Distance(rig2.rightHandTransform.position, rig1.headMesh.transform.position) < 0.25f)
                    {
                        ...
                        BetaSpawnSnowball(rig1.headMesh.transform.position + new Vector3(0f, 0.5f, 0f) + new Vector3(targetDirection.x, 0f, targetDirection.z).normalized / 1.7f, new Vector3(0f, -500f, 0f), 2, rig1.Creator.GetPlayerRef());
                        SetBoxingDelay(rig1);
                    }
                }
            }
        ...
        public static void SnowballDash()
        {
            foreach (VRRig rig in VRRigCache.ActiveRigs)
            {
                ...
                {
                    BetaSpawnSnowball(rig.headMesh.transform.position + new Vector3(0f, 0.5f, 0f) + new Vector3(-rig.headMesh.transform.forward.x, 0f, -rig.headMesh.transform.forward.z) * 1.5f, new Vector3(0f, -300f, 0f), 2, rig.Creator.GetPlayerRef());
                    SetBoxingDelay(rig);
                }
            }
        }
        ...
        public static void SnowballHighJump()
        {
            foreach (VRRig rig in VRRigCache.ActiveRigs)
            {
                ...
                {
                    BetaSpawnSnowball(rig.headMesh.transform.position + new Vector3(0f, -0.7f, 0f), new Vector3(0f, -500f, 0f), 2, rig.Creator.GetPlayerRef());
                    SetBoxingDelay(rig);
                }
            }
        }
        ...
        private static readonly Dictionary<VRRig, int> ghostedPlayers = new Dictionary<VRRig, int>();
        private static float masterGhostDelay;

        public static void MasterGhostGun()
        {
            if (!PhotonNetwork.IsMasterClient)
            {
                NotificationManager.SendNotification("<color=grey>[</color><color=red>ERROR</color><color=grey>]</color> You must be master client.");
                return;
            }
            ...
            if (GetGunInput(false))
            {
                ...
                if (GetGunInput(true) && Time.time > masterGhostDelay)
                {
                    masterGhostDelay = Time.time + 0.3f;
                    VRRig gunTarget = Ray.collider.GetComponentInParent<VRRig>();
                    if (gunTarget && !gunTarget.IsLocal())
                    {
                        ...
                        if (view != null)
                        {
                            NetPlayer targetPlayer = GetPlayerFromVRRig(gunTarget);
                            if (targetPlayer == null) return;
                            
                            if (ghostedPlayers.ContainsKey(gunTarget))
                            {
                                int viewID = ghostedPlayers[gunTarget];
                                PhotonNetwork.NetworkingClient.OpRaiseEvent(204, new Hashtable { { 0, viewID } }, 
                                    new RaiseEventOptions { TargetActors = new[] { targetPlayer.ActorNumber } }, SendOptions.SendReliable);
                                ghostedPlayers.Remove(gunTarget);
                            }
                            else
                            {
                                ghostedPlayers[gunTarget] = view.ViewID;
                                int[] targets = PhotonNetwork.PlayerList.Where(p => p.ActorNumber != targetPlayer.ActorNumber).Select(p => p.ActorNumber).ToArray();
                                PhotonNetwork.NetworkingClient.OpRaiseEvent(204, new Hashtable { { 0, view.ViewID } }, 
                                    new RaiseEventOptions { TargetActors = targets }, SendOptions.SendReliable);
                            }
                        }
                    }
                }
        ...
        public static void MasterGhostAll()
        {
            if (!PhotonNetwork.IsMasterClient)
            {
                NotificationManager.SendNotification("<color=grey>[</color><color=red>ERROR</color><color=grey>]</color> You must be master client.");
            ...
            foreach (VRRig rig in VRRigCache.ActiveRigs)
            {
                try
                {
                    if (rig != null && !rig.IsLocal())
                    {
                        ...
                        NetPlayer targetPlayer = GetPlayerFromVRRig(rig);
                        if (view != null && targetPlayer != null && !ghostedPlayers.ContainsKey(rig))
                        {
                            ghostedPlayers[rig] = view.ViewID;
                            int[] targets = PhotonNetwork.PlayerList.Where(p => p.ActorNumber != targetPlayer.ActorNumber).Select(p => p.ActorNumber).ToArray();
                            PhotonNetwork.NetworkingClient.OpRaiseEvent(204, new Hashtable { { 0, view.ViewID } }, 
                                new RaiseEventOptions { TargetActors = targets }, SendOptions.SendReliable);
                        }
        ...
        public static void MasterUnghostAll()
        {
            if (!PhotonNetwork.IsMasterClient) return;
            
            foreach (var kvp in ghostedPlayers.ToList())
            {
                try
                {
                    VRRig rig = kvp.Key;
        ...
        public static void MasterGhostAura()
        {
            ...
            foreach (VRRig rig in VRRigCache.ActiveRigs)
            {
                try
                {
                    if (rig != null && !rig.IsLocal() && Vector3.Distance(rig.transform.position, VRRig.LocalRig.transform.position) < 4f)
                    {
                        PhotonView view = GetPhotonViewFromVRRig(rig);
                        NetPlayer targetPlayer = GetPlayerFromVRRig(rig);
                        if (view != null && targetPlayer != null && !ghostedPlayers.ContainsKey(rig))
        ...
        public static void SpamMaterialAll()
        {
            if (!PhotonNetwork.IsMasterClient)
            {
                NotificationManager.SendNotification("<color=grey>[</color><color=red>ERROR</color><color=grey>]</color> You must be master client.");
